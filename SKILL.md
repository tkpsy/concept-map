...
2. **依存は本文に名前を書く。** 「生成エンジン が作る」「公開制御 が時刻を決める」のように書く。
   `[[リンク]]` は親から子への案内に使う。依存まで線にすると、全体像を辿る枝が埋もれる。
3. **ファイルは `docs/map/` 直下に並べる。** 階層はリンクで表す。日本語ファイル名でよい。
   `[[意味検索]]` は `意味検索.md` を指す。`/tick` のような名前はファイル名を `tick.md`、見出しを正式名にする。
...
1. **具体的な概念を短く説明する。** 名前に加えて「何がどう動くか」を書く。
   例：自動投稿＝アカウントが自分で投稿する、自動返信＝他の投稿に返信する、自動反応＝いいねやフォローをする。
2. **一緒に説明すると分かりやすいものを集め、一文にまとめる。**
   この例なら「アカウントが、人の操作なしにSNS上で活動する」。これが親の説明になる。
3. **説明に短い名前を付ける。** この例では「AI自動行動」。既存の呼び名が説明に合えば使う。
   「仕組み」だけで内容を予想しにくければ、何をする仕組みなのかまで名前に含める。
4. **名前から子を探せるか、親の説明から子の並びを理解できるか確かめる。**
   「自動返信の仕様を知りたい人が、AI自動行動を開きそうか」を考える。
   辿りにくければ名前かまとめ方を調整する。まとまりが見つからない概念は、今の親の直下に置いてよい。

まとめる観点は、その場所の説明に合わせる。「AI自動行動」は体験、「後処理」は処理の目的から作れる。
UI・実行プロセス・サービスなど、既に分かりやすいまとまりも使える。
同じディレクトリにあることや、同じ技術を使うことだけで親子が決まるわけではない。

親の候補が複数あるときは、**読者がその仕様を探す入口**を一つ選ぶ。他の関係は本文に名前を残す。
例えば「何が自動で起こるか」は AI自動行動、「文章をどう作るか」は生成エンジンで説明できる。
両者が関係していても、すべてを同じ枝の下に入れる必要はない。

初めて作るときや中間ノードに迷ったときは、[SNSの導出とファイル例](references/sns-example.md)を読む。
...
素の概念名       依存や関連の説明  生成エンジン が作る
素のパス         コードの場所      apps/web/  workers/foo.py
owner/repo#n    issue            example/social-app#27
```
